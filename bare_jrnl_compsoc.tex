...
\multirow{2}{*}{$Q$} & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}matrix used for previous hidden \\ layer to the current hidden layer\end{tabular}} & h,j\\\\
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \State {Initialize model $\Theta$: \{$\textbf{U}$, $\textbf{V}^I$, $\textbf{W}$,$\textbf{Q}$,$\textbf{P}$,\} randomly}
        \State iteration=0
        \Repeat
        \State $iteration\leftarrow iteration+1$;
            ...
            \EndFor
           \Until{converge or iteration>num}  \\
